...
namespace ECE141 {
  ...
  StatusResult parseAttributeOptions(Attribute &anAttribute, Tokenizer &aTokenizer) {
    ...
    while(theResult && options && aTokenizer.more()) {
      ...
      switch(theToken.type) {
        ...
        case TokenType::semicolon: //XXX-HACK
      ...
      if(Helpers::isDatatype(theToken.keyword)) {
        ...
        if(theResult) {
          ...
          if(!anAttribute.isValid()) {
            theResult.code=invalidAttribute;
          }
        }
  ...
  //USE: parse a comma-sep list of (unvalidated) identifiers; AUTO stop if keyword (or term)
  StatusResult SQLStatement::parseIdentifierList(StringList &aList, Tokenizer &aTokenizer) {
    ...
    while(theResult && aTokenizer.more()) {
      ...
      else if(theToken.type==TokenType::semicolon) {
        break;
      }
      else theResult.code=syntaxError;
    }
  ...
  AlterTableStatement::AlterTableStatement(SQLInterpreter &anInterpreter)
  : SQLStatement(anInterpreter, Keywords::alter_kw)  {}
  
  StatusResult AlterTableStatement::addAttribute(Tokenizer &aTokenizer) {
    StatusResult theResult{noError};
    Token &theToken=aTokenizer.current(); //identifier name...
    if(TokenType::identifier==theToken.type) {
      attribute.setName(theToken.data).setType(DataType::varchar_type);
      aTokenizer.next(); //skip name...
      theResult=parseAttribute(attribute, aTokenizer); //will already consume ')' for last attr...
    }
    return theResult;
  }

  StatusResult AlterTableStatement::dropAttribute(Tokenizer &aTokenizer) {
    StatusResult theResult{noError};
    return theResult;
  }
  
  StatusResult AlterTableStatement::modifyAttribute(Tokenizer &aTokenizer) {
    StatusResult theResult{noError};
    return theResult;
  }
    
  // USE: alter table |name| (|attrs|)...
  StatusResult AlterTableStatement::parse(Tokenizer &aTokenizer) {
    StatusResult theResult{syntaxError};
      ...
      if(Keywords::table_kw==theToken.keyword) {
        ...
        if(TokenType::identifier==theToken.type) {
          tableName=theToken.data; //hang on for later..
          aTokenizer.next(3); //skip ALTER TABLE...
          theToken=aTokenizer.current(); //should be ADD, DROP, CHANGE...
          action=theToken.data;
          aTokenizer.next();
          switch(theToken.keyword) {
            case Keywords::add_kw:    theResult = addAttribute(aTokenizer); break;
            case Keywords::drop_kw:   theResult = dropAttribute(aTokenizer); break;
            //case Keywords::modify_kw: theResult = modifyAttribute(aTokenizer); break;
            default: break;
          }
          if(theResult) skipIf(aTokenizer, TokenType::semicolon);
        }
      }
    }
    return theResult;
  ...
  //USE: called to execute an AlterTable statement...
  StatusResult AlterTableStatement::run(std::ostream &aStream) const {
    return interpreter.alterTable(tableName, attribute, action); //DB will "own" the entity...
  }
  ...
  StatusResult SelectStatement::parse(Tokenizer &aTokenizer) {
    ...
    if(3<aTokenizer.remaining()) {
      ...
      if(TokenType::operators==theToken.type) {
        aTokenizer.next(2); //skip *
        selectAll=true;
      }
      else {
        aTokenizer.next(); //move ahead...
      ...
      if(theResult) {
        ...
        if(Keywords::from_kw==theToken.keyword) {
          ...
          if(TokenType::identifier==theToken.type) {
            ...
            if(Database *theDatabase=interpreter.getActiveDatabase()) {
              if(Entity *theEntity = theDatabase->getEntity(name)) {
                aTokenizer.next(2); //FROM {tablename}...
