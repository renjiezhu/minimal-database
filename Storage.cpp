...
namespace ECE141 {
    ...
    //============================================================
    uint32_t BlockCache::decrement(uint32_t
                                   threshold) {
//        uint32_t minval = numLines;
        uint32_t minBlk = cache.begin()->first; //blocknum of first block
        for (
            auto entry
                : cache) {
            if (entry.second.second == 1) // priority
                minBlk = entry.first; // blocknum
            if (entry.second.second > threshold) // priority
                entry.second.second--;
        }
        return
                minBlk;
    ...
    bool BlockCache::contains(uint32_t aBlockNum)//return true if present
    {
        return cache.find(aBlockNum) != cache.end();
    }

    Block &BlockCache::get(uint32_t aBlockNum) //retrieve from cache (may throw exception)
    {
        this->decrement(cache[aBlockNum].second);
        return cache.at(aBlockNum).first;
    }

    void BlockCache::put(uint32_t aBlockNum, const Block &aBlock)  //save to cache
    {
        if (this->contains(aBlockNum)) {
            if (cache[aBlockNum].second < cache.size()) // else this is already highest priority entry
                this->decrement(cache[aBlockNum].second);
            cache[aBlockNum] = std::make_pair(aBlock, cache.size());
        } else if (cache.size() < numLines) // Adding to cache with free slots
            cache[aBlockNum] = std::make_pair(aBlock, cache.size());
        else // New entry to full cache
        {
            cache.erase(this->decrement(0));
            cache[aBlockNum] = std::make_pair(aBlock, numLines);
        }
    }
...
// USE: read data a given block # ---------------------------------------
    StatusResult Storage::readBlock(int aBlockNumber, Block &aBlock, std::size_t aBlockSize) {
        //  Modify this code assuming that there's a cache
        if (isCachePresent) {
            if (Cache.contains(aBlockNumber)) {
                ...
                //  It's a miss! Read from Storage and put it in the cache!
