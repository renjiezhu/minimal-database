...
namespace ECE141 {
    ...
    class UnixFolderReader : public MacFolderReader {
    public:
      UnixFolderReader(const char *aPath) : MacFolderReader(aPath) {} //should just work; not tested...
    };
    ...
    public:
        ...
        void each(folder_callback aCallback, const char *anExtension) const {
            ...
            if (int theCount = scandir(path.c_str(), &theFiles, NULL, alphasort)) {
                for (int i = 0; i < theCount; i++) {
                    struct dirent *theEntry = theFiles[i];
